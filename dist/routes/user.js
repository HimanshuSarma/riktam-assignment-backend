...
userRouter.post(`/signup`, signupController.validation, signupController.handler);
userRouter.post(`/login`, loginController.validation, loginController.handler);
// All POST routes here end...
// All GET routes here start...
userRouter.get('/test', (req, res) => {
    res.status(200).send('test');
});
userRouter.get(`/all`, getAllUsersController.handler);
userRouter.get(`/all-in-given-chatroom`, extractDataAndCallVerifyToken, getAllUsersInGivenChatRoomController.validation, getAllUsersInGivenChatRoomController.handler);
